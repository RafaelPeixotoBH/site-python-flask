...
# --- 1. CORREÇÃO DE REDE OBRIGATÓRIA PARA O RENDER ---
# Força o uso de IPv4 para conectar ao Brevo/Gmail, evitando erro [Errno 110]
...
class User(UserMixin, db.Model): 
    ...
    # Relacionamento: Um usuário tem vários acessos
...
def home():
    # Rota principal: Lista de nomes simples
...
def recuperar_senha():
    if request.method == 'POST':
        ...
        try:
            ...
            # Captura erro de conexão para não travar o servidor
...
def dashboard():
    ...
    # Busca dados para exibir no HTML
    usuarios = User.query.all()
    # Pega os últimos 50 logins (mais recentes primeiro)
...
def setup_banco():
    with app.app_context():
        db.drop_all()
        db.create_all()
    return "Banco Resetado! Tabelas 'user' e 'login_history' criadas com sucesso."
...
def criar_admin():
    if not User.query.filter_by(username='admin').first():
        ...
        db.session.add(adm)
        db.session.commit()
        return f"Admin criado com sucesso! Login: admin | Senha: 123"
